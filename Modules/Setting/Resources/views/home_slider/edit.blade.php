    ...
    <div class="content-wrapper">
        ...
        <section class="content-header">
            <div class="container-fluid">
                <div class="row mb-2">
                    <div class="col-sm-6">
                        <h1>{{$custom[strtolower('home_slider')]??""}}</h1>
                    </div>
                    <div class="col-sm-6">
                        <ol class="breadcrumb float-sm-right">
                            <li class="breadcrumb-item"><a
                                    href="{{route('admin.dashboard')}}">{{$custom[strtolower('Home')]??""}}</a></li>
                            <li class="breadcrumb-item active">{{$custom[strtolower('home_slider')]??""}}</li>
                        </ol>
                    </div>
                </div>
            </div><!-- /.container-fluid -->
                ...
                <div class="row">
                    ...
                    <div class="col-md-12">
                        ...
                        <div class="card card-primary">
                            ...
                            <form action="{{route('home_slider.update',$data->id)}}" method="post" id="edit" enctype="multipart/form-data">
                                @csrf
                                <div class="card-body">
                                    ...
                                    @endforeach
                                        @foreach($language as $lang)
                                            <div class="form-group{{ $errors->has('description['.$lang->code.']') ? ' is-invalid' : "" }}">
                                                ...
                                                <textarea type="text" name="description[{{$lang->code}}]" class="form-control" id="{{$lang->code}}"
                                                          placeholder="{{$custom[strtolower('Enter_Description')]??"lang not found"}} {{$lang->name}}">{{$data->descriptionValue()[$lang->code]??""}}</textarea>
                                            </div>
                                        @endforeach
                                    <div class="form-group{{ $errors->has('order') ? ' is-invalid' : "" }}">
                                        ...
                                        <input type="text" name="order" class="form-control" id="order"
                                               value="{{$data->order}}" placeholder="{{$custom[strtolower('Enter_Order')]??"lang not found"}}">
                                    </div>
                                        <div class="form-group{{ $errors->has('url') ? ' is-invalid' : "" }}">
                                        <label for="url">{{$custom[strtolower('url')]??"lang not found"}}</label>
...
@section('script_style')
    {!! JsValidator::formRequest('Modules\Setting\Http\Requests\HomeSlider\EditRequest','#edit') !!}
@endsection
