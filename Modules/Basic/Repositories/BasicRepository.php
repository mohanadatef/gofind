...
abstract class BasicRepository
{
     ...
     * trash is relation with delete value
     */
    public function all($search = [], $column = ['*'], $withRelations = [], $recursiveRel = [], $moreConditionForFirstLevel = [], $trash = false, $pluck = [],
                        $orderBy = [], $get = '', $skip = null, $limit = null, $pagination = false, $perPage = 0)
    {
        $query = $this->allQuery($search, $skip, $limit);

        if ($trash == true) {
            $query = $query->onlyTrashed();
        } elseif ($trash == 'withTrashed') {
    ...
    public function find($id, $column = ['*'], $withRelations = [], $trash = false)
    {
