                ...
                <div class="row">
                    ...
                    <div class="col-md-12">
                        ...
                        <div class="card card-primary">
                            ...
                            <form action="{{route('role.update',$data->id)}}" method="post" id="edit" enctype="multipart/form-data">
                                @csrf
                                <div class="card-body">
                                        ...
                                        <div class="form-group{{ $errors->has('permission') ? ' has-error' : "" }}">
                                            ...
                                            <select class="duallistbox" multiple="multiple" name="permission[]">
                                                @foreach($permission as $key => $pe)
                                                    <option @foreach($data->role_permission as  $rp) @if($rp->id ==$pe->id) selected   @endif @endforeach value="{{$pe->id}}">{{$pe->display_name->value}}</option>
                                                @endforeach
                                            </select>
                                        </div>
                                    <!-- /.form-group -->
...
@section('script_style')
    ...
    {!! JsValidator::formRequest('Modules\Acl\Http\Requests\Role\EditRequest','#edit') !!}
@endsection
