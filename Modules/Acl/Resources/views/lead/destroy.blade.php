    ...
    <div class="content-wrapper">
        ...
        <section class="content-header">
            <div class="container-fluid">
                <div class="row mb-2">
                    <div class="col-sm-6">
                        <h1>{{$custom[strtolower('lead')]??"lang not found"}}</h1>
                    </div>
                    <div class="col-sm-6">
                        <ol class="breadcrumb float-sm-right">
                            <li class="breadcrumb-item"><a href="{{route('admin.dashboard')}}">{{$custom[strtolower('Home')]??"lang not found"}}</a></li>
                            <li class="breadcrumb-item active">{{$custom[strtolower('lead')]??"lang not found"}}</li>
                        </ol>
                    </div>
                </div>
            </div><!-- /.container-fluid -->
                                    ...
                                    @forelse($datas as $data)
                                        <tr id="data-{{$data->id}}">
                                            ...
                                            <td>
                                                ...
                                                @endpermission
                                                @permission('lead-remove')
                                                <button type="button" class="btn btn-outline-primary btn-block btn-sm"
                                                        onclick="selectItem({{$data->id}})" data-toggle="modal"
                                                        data-target="#modal-remove">
                                                    <i class="fa fa-edit"></i> {{$custom[strtolower('remove')]??"lang not found"}}
                                                </button>
                                                @endpermission
                                            </td>
                                        </tr>
